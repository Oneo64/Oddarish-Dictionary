...
function get_noun_declension(w, t) {
	...
	if (word.endsWith("nir")) {
		...
		if (word.endsWith("ir")) {
		...
		} else if ((word.endsWith("rr") || word.endsWith("gr") || word.endsWith("fr") || word.endsWith("lr")) && !word.endsWith("ngr") && !word.endsWith("llr")) {
			for (var i = 0; i < 16; i++) {
			...
			for (var i = 0; i < 16; i++) {
				if (noun_declensions.masculine_nn2[i] == "") {
					declension.push(word);
				} else {
					declension.push(word.substring(0, word.length - 1) + noun_declensions.masculine_nn2[i]);
				}
			}
